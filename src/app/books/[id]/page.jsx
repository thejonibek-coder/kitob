...
import { Star, Heart, BookOpen, Calendar, FileText, Globe, Hash, ArrowLeft } from "lucide-react";
...
export default function BookDetailPage() {
  ...
  if (!book) {
    return (
      <div className="max-w-7xl mx-auto section-padding py-20 text-center">
        ...
        <Link href="/books" className="text-primary-600 hover:underline">Back to books</Link>
      </div>
    );
  }
  ...
  />
        ...
        <div className="lg:col-span-2">
          <span className="inline-block px-3 py-1 rounded-lg bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400 text-sm font-medium mb-4">
            {book.category}
          </span>
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-3">{book.title}</h1>
          <p className="text-lg text-gray-500 dark:text-gray-400 mb-4">by {book.author}</p>

          <div className="flex items-center gap-4 mb-6">
            <div className="flex items-center gap-1.5">
              <Star size={20} className="text-amber-400 fill-amber-400" />
              <span className="font-semibold text-gray-900 dark:text-white text-lg">{book.rating}</span>
              <span>
  {book.rating} ⭐
  ({(book.reviews ?? 0).toLocaleString()} reviews)
            ...
            {book.price === 0 ? (
              <span className="px-3 py-1 rounded-lg bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-sm font-semibold">
                Free
              </span>
            ) : (
              <span className="text-2xl font-bold text-gray-900 dark:text-white">${book.price}</span>
            )}
          </div>

          <p className="text-gray-600 dark:text-gray-300 leading-relaxed mb-8 text-lg">{book.description}</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className="p-4 rounded-xl bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800">
              ...
              <p className="font-medium text-gray-900 dark:text-white">{book.published}</p>
            </div>
            <div className="p-4 rounded-xl bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800">
            ...
            <div className="p-4 rounded-xl bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800">
              ...
              <p className="font-medium text-gray-900 dark:text-white text-sm">{book.isbn}</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <button
              ...
              className={`btn-secondary inline-flex items-center gap-2 ${favorites.includes(book.id) ? "text-red-600 dark:text-red-400" : ""}`}
            >
              <Heart size={18} fill={favorites.includes(book.id) ? "currentColor" : "none"} />
              {favorites.includes(book.id) ? "In Favorites" : "Add to Favorites"}
            </button>
          </div>
        </div>
      </div>
